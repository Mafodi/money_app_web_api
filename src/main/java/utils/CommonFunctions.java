...
public class CommonFunctions {
    ...
    /* ***********************************************************************
       Author-CC311676/Halatedzi
     ************************************************************************/
    public void clickConfirm() {

        driverWait.until(ExpectedConditions.visibilityOfAllElements( wdriver.findElement(By.id("confirm"))));

        WebElement confirm = wdriver.findElement(By.id("confirm"));

        ((JavascriptExecutor) wdriver).executeScript("arguments[0].scrollIntoView(true)", confirm);

        confirm.click();

        try {

            while(driverWait.until(ExpectedConditions.visibilityOfElementLocated(By.id("try_again"))).isDisplayed()){

                WebElement tryAgain = wdriver.findElement(By.id("try_again"));

                ((JavascriptExecutor) wdriver).executeScript("arguments[0].scrollIntoView(true)", tryAgain);
    ...
    public void clickAllCheckBoxes() {
        try {
            List<WebElement> checkBoxList = wdriver.findElements(By.xpath("//input[@type='checkbox']"));

            if (!checkBoxList.isEmpty()) {
                for (WebElement checkBox : checkBoxList) {
                    try {
                        if (!checkBox.isSelected()) {
                            ((JavascriptExecutor) wdriver).executeScript("arguments[0].click();", checkBox);
                        }
                    } catch (Exception e) {
                    }
                }
    ...
    public boolean StringValidation(String elementText) {
        try {
            String actualText = "";
            String expectedText ="";
            ...
            this.secondsDelay(2);
            driverWait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[contains(text(),\'"+elementText+"\')]")));
            WebElement x = wdriver.findElement(By.xpath("//*[contains(text(),\'"+elementText+"\')]"));
            ...
            Assert.fail("Couldn't match strings, Validation failed!");
            return false;
        }
    }

    public boolean findAndClickButtonByText(String elementText) {
        try {
            wdriver.switchTo().activeElement();
            ...
            ((JavascriptExecutor) wdriver).executeScript("arguments[0].scrollIntoView(true)", x);
    ...
    public void selectOffer(String offerDescription ) {

        this.secondsDelay(5);

        List<WebElement> options = wdriver.findElements(By.className("description"));
        ...
        if(options.size() > 0){
            ...
            for (WebElement option : options)
            {
                ...
                if (option.getText().contains(offerDescription))
                {
                    ((JavascriptExecutor) wdriver).executeScript("arguments[0].scrollIntoView(true)", option);
                    option.click();
                    break;
                }
            }
    ...
    public boolean waitForElementVisibleByXpath(int seconds, String elementXpath) {
        try {
