...
public class FND_Transfer_From_Savings_To_Current_Account extends DriverFactory {

    WebDriverWait wait=new WebDriverWait(getIOSDriver(),30);
    private IOSCommonMethods basepage = new IOSCommonMethods();
    private static Logger log = Logger.getLogger(platforms.ui.mobile.ios.StepDefinitions.FND_Transfer_From_Current_To_Savings_Account.class);
    iOSTransferScreen transferScreen=new iOSTransferScreen(driver);
    iOSTransfer_Flow transferFlow=new iOSTransfer_Flow();
    TouchAction touchAction=new TouchAction(driver);

    public FND_Transfer_From_Savings_To_Current_Account(){}

    @Given("^Tsc_User is on the Transfer screen ios app$")
    public void tsc_userIsOnTheTransferScreenIosApp() throws Throwable {
        transferFlow.Navigate_To_Transfer_Screen();
    }

    @When("^Tsc_User enters amount to transfer ios app$")
    public void tsc_userEntersAmountToTransferIosApp() throws InterruptedException {
        Thread.sleep(2000);
    ...
    public void tsc_userSelectsFromAccountIosApp() throws InterruptedException {
    ...
    public void tsc_userSelectsToAccountIosApp() {
        basepage.horizontalSwipeByPercentage(0.6,0.1,0.3);
        basepage.horizontalSwipeByPercentage(0.6,0.1,0.3);
    }

    @Then("^Tsc_User Clicking on next should take user to review transfer screen ios app$")
    public void tsc_userClickingOnNextShouldTakeUserToReviewTransferScreenIosApp() throws InterruptedException {
        Thread.sleep(1000);
    ...
    public void tsc_userShouldBeAbleToMakeTransferSuccessfullyIosApp() throws InterruptedException {
        Thread.sleep(2000);
