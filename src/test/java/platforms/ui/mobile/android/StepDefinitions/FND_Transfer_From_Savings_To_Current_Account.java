...
public class FND_Transfer_From_Savings_To_Current_Account extends DriverFactory {
    ...
    public void tsc_userIsOnTheTransferScreen() throws Throwable {
        transferFlow.Navigate_To_Transfer_Screen();
    }

    @When("^Tsc_User enters amount to transfer$")
    public void tsc_userEntersAmountToTransfer() throws InterruptedException {
        Thread.sleep(2000);
        transferScreen.Transfer_amount.sendKeys("1555");
    }

    @And("^Tsc_User selects from account$")
    public void tsc_userSelectsFromAccount() throws InterruptedException {
        try{
    ...
    public void tsc_userSelectsToAccount() throws InterruptedException {
        try{
            AndroidCommonMethods.directionSwipe(AndroidCommonMethods.DIRECTION.RIGHT);
        }catch(Exception e){
            log.info("Unable to swipe",e);
    ...
    public void tsc_userClickingOnNextShouldTakeUserToReviewTransferScreen() {
        transferScreen.Transfer_button.click();
    }

    @And("^Tsc_User should be able to make transfer successfully$")
    public void tsc_userShouldBeAbleToMakeTransferSuccessfully() {
