...
public class FND_Make_A_Future_Dated_Payment_from_current_to_Existing_nedbank_Investment extends DriverFactory {
    ...
    public FND_Make_A_Future_Dated_Payment_from_current_to_Existing_nedbank_Investment(){}

    @Given("^FD_User_iv is on the Payment screen$")
    public void fd_user_ivIsOnThePaymentScreen() throws Throwable {
        paymentFlow.Navigate_To_Payment_Screen();
    }

    @When("^FD_User_iv navigates to Single Payment screen$")
    public void fd_user_ivNavigatesToSinglePaymentScreen() {
        payScreen.Transact_single_pay.click();
        log.info("single payment selected");
    }

    @And("^FD_User_iv selects recipients icon$")
    public void fd_user_ivSelectsRecipientsIcon() {
        payScreen.Transact_Pay_Beneficiary_list_icon.click();
    }

    @And("^FD_User_iv should be able to search \"([^\"]*)\" and select$")
    public void fd_user_ivShouldBeAbleToSearchAndSelect(String recipientsname) throws Throwable {
        paymentFlow.Select_Beneficiary(recipientsname);
    }

    @Then("^FD_User_iv Clicking on next should take me to next screenThen PEM_AcctUser_schedule clicking on next should take me to next screen and select a future date$")
    public void fd_user_ivClickingOnNextShouldTakeMeToNextScreenThenPEM_AcctUser_scheduleClickingOnNextShouldTakeMeToNextScreenAndSelectAFutureDate() throws InterruptedException {
        paymentFlow.Enter_Payment_Amount();
        paymentFlow.Select_future_date();
    }

    @And("^FD_User_iv should be able to make payment successfully$")
    public void fd_user_ivShouldBeAbleToMakePaymentSuccessfully() throws InterruptedException {
        Thread.sleep(2000);
